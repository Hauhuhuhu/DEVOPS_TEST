...
- [ ] Cashier can select a Variant and its Modifiers from the POS UI.
- [ ] Cart calculates total price dynamically based on Variant base price and selected modifiers.
- [ ] Submitting the order creates an `OrderItem` containing the `variant_id` and a `selected_modifiers` JSON payload.
- [ ] An automatic `OUT` transaction is created in the `InventoryTransaction` ledger for the sold Variant.
- [ ] The Variant's cached stock is updated, even if it results in a negative value (Negative Stock).
- [ ] API integration tests confirm the entire checkout flow and ledger reduction.
