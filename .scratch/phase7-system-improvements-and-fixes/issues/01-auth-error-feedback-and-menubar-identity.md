...
- [ ] Authentication API returns HTTP 401 with informative error message on incorrect password or nonexistent email
- [ ] Authentication API returns HTTP 401 when account is disabled
- [ ] Authentication login and refresh endpoints return the user's display name in the auth response
- [ ] Login screen renders an alert banner displaying the server error message upon failed authentication
- [ ] Submit button shows loading state and prevents double submission during authentication
- [ ] Desktop navigation bar displays user name alongside a distinct role badge for Admin vs Staff
