...
- [ ] Attempting to delete a COMPLETED order returns HTTP 400 Bad Request with a clear explanation
- [ ] Deleting a PENDING order creates compensating inventory IN transactions for all line items and updates cached stock
- [ ] Deleting a PENDING order decrements the usage count of any applied promotion coupon
- [ ] Deleting a PENDING order reverts customer lifetime spending and order count metrics in CRM
- [ ] Deleting a PENDING order triggers remote PayOS checkout link cancellation if applicable
- [ ] Deleting an already CANCELLED order deletes the database record without repeating stock or promo reversals
- [ ] Deleting an order emits an ActivityLog record with action DELETE and entityType ORDER
