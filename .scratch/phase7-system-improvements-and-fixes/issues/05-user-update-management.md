...
- [ ] User update API endpoint updates user display name, email, and role
- [ ] User update securely encodes new password if provided, or leaves existing password intact if left blank
- [ ] User update validates email uniqueness against other users
- [ ] Updating a user emits an ActivityLog record with action UPDATE and entityType USER
- [ ] User list item displays an Edit action button opening a dedicated Edit User modal
- [ ] Modal allows changing name, email, role, and optional password with clear form validation
- [ ] Successful update refreshes user query cache and displays confirmation toast
